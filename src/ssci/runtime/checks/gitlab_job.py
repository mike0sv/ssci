...
class PipelineFinished(Check, ABC):
    ...
    def check(self, project: Deployment, commit: str):
        ...
        while time.time() - self.pipeline_timeout < start:
            ...
            if pipeline.status == 'success':
                ...
                raise CheckNotPassed(f'Unknown gitlab pipeline status {pipeline.web_url}')
            time.sleep(self.poll_timeout)

    def find_pipeline(self, commit: str) -> ProjectPipeline:
        start = time.time()
        while time.time() - self.start_timeout < start:
            pipelines = [p for p in self.gl_project.pipelines.list() if p.sha.startswith(commit)]
            if len(pipelines) > 0:
                return pipelines[0]
            time.sleep(self.poll_timeout)
    ...
    def gl_project(self) -> Project:
        gl_project = [p for p in self.gl.projects.list() if p.http_url_to_repo.startswith(self.project_url)]
        if len(gl_project) == 0:
